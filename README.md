...
```
.
├── main.py                  # Entry point: runs all 18 VQE configurations
├── hamiltonian.py           # PySCF integral generation + second quantization
├── mapping.py               # Jordan–Wigner, Parity, Bravyi–Kitaev mappings
├── ansatz.py                # UCCSD, PUCCSD, PUCCD circuit builders
├── vqe_runner.py            # VQE loop with SLSQP / COBYLA optimizers
├── results/
│   ├── fixed_point/         # Energy, error, iterations, runtime (18 runs)
│   └── distance_sweep/      # Supplementary validation over d ∈ {0.50–1.50 Å}
├── plots/
│   ├── accuracy_comparison.png
│   ├── iteration_comparison.png
│   └── runtime_comparison.png
├── report/
│   └── H2_orbitals_structure.pdf
└── README.md
```
 
> ⚠️ Adapt the structure above to match your actual file layout.
...
pip install pyscf qiskit qiskit-nature qiskit-algorithms scipy numpy matplotlib
```
...
- 3 mappings × 2 optimizers × 3 ansatz = **18 runs**
- Fixed geometry: d = 0.735 Å (STO-3G equilibrium)
...
qubit_op = build_hamiltonian(mapping="jordan_wigner")
result = run_vqe(qubit_op, ansatz="UCCSD", optimizer="SLSQP")
...
```bash
python distance_sweep.py --mapping jordan_wigner --optimizer SLSQP --ansatz UCCSD
```
